 ...
 * \n
 * This class provides methods for iterating over a FEN string, skipping whitespace, and accessing the current character.
 * It also provides methods for checking if there are any characters left in the string and if the current character is a whitespace.
 */
...
// constructor
     ...
     */
    constexpr inline explicit FenStringReader(std::string_view str) noexcept : str(str), offset(0), size(static_cast<int>(str.size())) {
#ifndef NO_ASSERTS
 ...
 */
    constexpr inline void operator+=(int i) noexcept {
        offset += i;
    };
 ...
 */
    [[nodiscard]] constexpr inline bool hasCharsLeft() const noexcept {
 ...
 */
    [[nodiscard]] constexpr inline int amountCharsLeft() const noexcept {
        return size - offset;
    }
 ...
 */
    [[nodiscard]] constexpr inline int getCurrentOffset() const noexcept {
        return offset;
    }
...
struct Fen {
     ...
     */
    constexpr explicit Fen() = default;
