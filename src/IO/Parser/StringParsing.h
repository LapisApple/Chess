 ...
 */
constexpr inline uint8_t charToInt(char ch) noexcept { return ch - '0'; }
 ...
 *
 * The string is expected to represent a move in a chess game, in the format
 *"[Piece][Source][Capture][Target][Promotion]". (the same format as in the exercises)
 *
 ...
 * castling: Ke1c1 \n
 */
constexpr SlimOptional<UserMove> convert_string_to_move(const std::string_view& input) {
  // check no out of bounds access necessary
  if (input.size() < 5) return SlimOptional<UserMove>::nullopt();
  ...
  // because the second part of the second position might access input[5] if move.is_capture
  if (move.is_capture && input.size() < 6) return SlimOptional<UserMove>::nullopt();
  ...
  if (input[input.size() - 2] == '=') {
    ...
    if (promote.data.type == PieceType::PAWN || promote.data.type == PieceType::KING)
      return SlimOptional<UserMove>::nullopt();
