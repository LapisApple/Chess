...
struct UserMove {
     ...
     */
    explicit constexpr UserMove() noexcept : piece(Piece::getEmpty()), from(-1), to(-1),
                                             is_capture(false), promote(OptionalPieceType::nullopt()) {}
     ...
     * @return true if the User move is the same as the normal move when disregarding specialMove data, and false otherwise
     */
    [[nodiscard]] constexpr inline bool equals(Move other, bool other_is_capture) const {
        return this->piece == other.piece &&
               this->from == other.from &&
               this->to == other.to &&
               this->promote.data == other.promote.data &&
               this->is_capture == other_is_capture;
 ...
 */
template<> constexpr inline SlimOptional<UserMove> SlimOptional<UserMove>::nullopt() noexcept {
 ...
 */
template<> constexpr inline bool SlimOptional<UserMove>::has_value() const noexcept {
    const bool result_condition = data.to >= 0 && data.to < 64;
#ifndef NO_ASSERTS
    const bool extensive_condition = (
            data.to >= 0 && data.to < 64 &&
            data.from >= 0 && data.from < 64 &&
