 ...
 * The first element is the minimum value (for the black player), and the second element is the maximum value (for the white player).
 */
...
SlimOptional<Move> MinMax(const Board& board, int depth, Team::Team player, int currentTurn) {
    ...
    if (!board.positions.hasPiece(Team::WHITE, PieceType::KING) || !board.positions.hasPiece(Team::BLACK, PieceType::KING)) return SlimOptional<Move>::nullopt();
    ...
    if (player == Team::WHITE) {
        auto maxEval = INT32_MIN;
        for (Move nextMove: move_list) {
            int eval = MinMax_impl(board, depth - 1, Team::BLACK, nextMove, alpha, beta, currentTurn + 1);
            if (eval > maxEval) {
                maxEval =  eval;
        ...
        auto minEval = INT32_MAX;
        for (Move nextMove: move_list) {
            int eval = MinMax_impl(board, depth - 1, Team::WHITE, nextMove, alpha, beta, currentTurn + 1);
            if (eval < minEval) {
                minEval = eval;
                best_move = SlimOptional(nextMove);
            }
