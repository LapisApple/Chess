 ...
 * @param offset The offset to add to the current position of the piece (i.e. the 2D vector from the starting position to the end position of the move)
 ...
 *
 * This function generates all possible moves for a sliding piece (rook, bishop, queen) in a certain direction on a given board.
 * It does this by starting from the current position of the piece and moving in the direction specified by the step parameter,
 * until it reaches the edge of the board or encounters another piece.
 */
template<typename fn>
constexpr static void get_possible_move_linear_search(const Board& board, fn forEachPossible, Piece sliding_piece, Vec2 from, Vec2 step) {
 ...
 */
constexpr static bool path_is_free_between_king_and_castle(const Board& board, int king_pos, int castle_pos, int8_t step) {
 ...
 * @param step the direction from the king to the castle (i.e. 1 for king side and -1 for queen side)
 *
 * @note should be called only after it is certain that the path between the king and castling taking part in castling is free \n
 * (makes a copy of the board due to not wanting to const_cast the const qualifier away and end up with unexpected behaviour)
...
namespace PossibleMoves {
    ...
    void getPossiblePawnMove(const Board& board, std::vector<Move>& moveVec, Piece pawn, int8_t pawn_pos) {
        ...
        for (struct { int i; Vec2 to2D; } i = {0, pawn_pos_2D + pawnMovement}; (i.i < (1 + isStartLine)) && !i.to2D.outsideBoard(); ++i.i, i.to2D += pawnMovement) {
    ...
    void getPossibleKingMove(const Board& board, std::vector<Move>& moveVec, Piece king, int8_t king_pos) {
        ...
        const int8_t teamOffset = king.team == Team::WHITE ? ChessConstants::start_white_queen_side_castle_pos : ChessConstants::start_black_queen_side_castle_pos;
     ...
     */
    void getPossibleCastleMove(const Board& board, std::vector<Move>& moveVec, Piece castle, int8_t castle_pos, SlimOptional<SpecialMove> specialMove) {
