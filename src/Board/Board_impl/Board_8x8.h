...
class Board_8x8 {
 ...
 public:
   ...
   */
  constexpr Board_8x8() noexcept : Board_8x8("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR") {}

  /**
   * @brief constructs a Board_grid with the given parameters
   * @param fen_board the board part of the FEN as given by Fen.board, meaning a legal board without any illegal
  ...
  explicit constexpr Board_8x8(const std::string_view& fen_board) noexcept {
    int offset = 0;
    for (uint64_t i = 0; i < fen_board.size(); ++i) {
      char ch = fen_board[i];
