 ...
 * each team gets 2 following bits with the left bit being castling queen side and the right being castle right king
 * side that's also why the specialMoves around castling have these magic values
 */
...
class Board_Extra {
 private:
   ...
   * defined for the inputs: 0b00, 0b01, 0b10, 0b11, undefined for all other inputs (flags can just directly be taken
   * from the corresponding SpecialMove enums)
   */
 ...
 public:
   ...
   */
  constexpr Board_Extra() noexcept : castling(0b1111), passant_pos(ChessPos::nullopt()) {}
   ...
   * @param en_passant_pos an Optional of the possible En Passant Position (sth. like the "shadow" of where it can be
   * taken)
   ...
   */
  explicit constexpr Board_Extra(uint8_t castling_right_flags, ChessPos en_passant_pos) noexcept
      : castling(castling_right_flags), passant_pos(en_passant_pos) {}
   ...
   * @note can also be used like a condition due to the return being 0 when there are no castling flags for that
   * position and >0 if there are castling flags
   */
   ...
   */
  [[nodiscard]] constexpr ChessPos getPassantPos() const noexcept { return passant_pos; }
  ...
  [[nodiscard]] constexpr ChessPos getPosOfPassantablePiece() const noexcept {
    ...
    const int detailed_offset = row == ChessConstants::black_en_passant_row
                                    ? 8
                                    : ((row == ChessConstants::white_en_passant_row) ? -8 : ROW_ERROR_VALUE);
  ...
  constexpr void movePiece(Move move, Piece piece_captured) noexcept {
    ...
    if (piece_captured.type == PieceType::CASTLE) {
      ...
      // works because the return of getCastlingRights() only has '1' on bits that are currently on in the castling
      // variable meaning that this xor turns of the correct bits and does not turn any back on
      castling ^= lost_flags;
    }
  }
};
