...
TEST(Team_getEnemyTeam, asset_catch_on_undefined_input) { ASSERT_DEBUG_DEATH(Team::getEnemyTeam(Team::NONE), ""); }
...
TEST(Piece_Struct, equality_correctness) {
  ...
  // just test all possible combinations that should be different
    ...
    for (int other_teams = teams; other_teams < 3; ++other_teams) {
      for (int i = 0; i < PIECETYPE_AMOUNT; ++i) {
        for (int j = i + 1; j < PIECETYPE_AMOUNT; ++j) {
          ASSERT_FALSE(Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)) ==
                       Piece(static_cast<Team::Team>(other_teams), static_cast<PieceType::PieceType>(j)));
          ASSERT_TRUE(Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)) !=
                      Piece(static_cast<Team::Team>(other_teams), static_cast<PieceType::PieceType>(j)));
        }
      }
    }
  }
  // just test all possible combinations that should be same
  for (int teams = 0; teams < 3; ++teams) {
    for (int i = 0; i < PIECETYPE_AMOUNT; ++i) {
      ASSERT_TRUE(Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)) ==
                  Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)));
      ASSERT_FALSE(Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)) !=
                   Piece(static_cast<Team::Team>(teams), static_cast<PieceType::PieceType>(i)));
    }
  }
